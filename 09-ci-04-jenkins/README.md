...
---
### 7) Указываем рабочий каталог и команду запуска процесса JAVA на узле-агенте. Путь /opt/jenkins_agent/ берем из переменной jenkins_agent_dir проекта: ( 0 ) .
Задаем чиcло executors-сборщиков равным кол-ву виртуальных CPU.
...
### 10) Установим подключаемые модули, необходимые для поддержки Docker - "Docker plugin" и "Docker Pipeline" и перезагружаем master-хост
...
Можно сразу использовать готовые ключи созданные через сценарий  в папке ~/.ssh  или сгенерировать новые.
