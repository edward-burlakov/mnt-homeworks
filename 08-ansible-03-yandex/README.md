...
### 1) Создаем  в Yandex Cloud   три хоста  clickhouse-01, vector-01  , lighthouse-01 .
       ...
       В атрибутах хостов в поле  ssh ключа копируем и вставляем public ключ с локального хоста, на котором запускаем ansible.
...
### 4) Если всё  прошло успешно входим на сервер и проверяем статус сервера clickhouse
       ...
       #
---
### 5) Проверяем подключение к данной БД  с помощью клиента clickhouse-client  
...
Цели плейбука:      
        ...
        - На втором  хосте на платформе YC устанвоаливает  сервер Vector
